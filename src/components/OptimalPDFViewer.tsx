...
function EnhancedSidebar({ isOpen, activeTab = 'search', onTabChange }: { 
  isOpen: boolean; 
  activeTab?: 'thumbnails' | 'search';
  onTabChange?: (tab: 'thumbnails' | 'search') => void;
}) {
  const [localActiveTab, setLocalActiveTab] = useState<'thumbnails' | 'search'>(activeTab);
  ...
  // Update local tab state when activeTab prop changes
  useEffect(() => {
    setLocalActiveTab(activeTab);
  }, [activeTab]);

  // Handle tab changes
  const handleTabChange = (tab: 'thumbnails' | 'search') => {
      ...
      <div className="py-3">
        ...
        >
          <button 
            type="button" 
            role="radio" 
            aria-checked={localActiveTab === 'thumbnails'}
            data-state={localActiveTab === 'thumbnails' ? "on" : "off"}
            ...
            onClick={() => handleTabChange('thumbnails')}
          >
            Thumbnails
          </button>
          <button 
            type="button" 
            role="radio" 
            aria-checked={localActiveTab === 'search'}
            data-state={localActiveTab === 'search' ? "on" : "off"}
            className="inline-flex items-center justify-center rounded-md text-sm text-textGray4 font-medium ring-offset-background transition-colors hover:bg-muted hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=on]:bg-muted data-[state=on]:text-accent-foreground [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 bg-transparent h-7 px-3 flex-1"
...
export default function OptimalPDFViewer({ 
  ...
  const [sidebarActiveTab, setSidebarActiveTab] = useState<'thumbnails' | 'search'>('thumbnails');
  ...
  // Function to open sidebar with specific tab active
  const openSidebarWithTab = (tab: 'thumbnails' | 'search') => {
    // If sidebar is open and already showing the selected tab, close it
            ...
            <Search>
              ...
              <div className="sticky top-0 z-[50] p-2 flex items-center justify-between border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
                <div className="flex items-center space-x-2">
                  <button 
                    onClick={() => openSidebarWithTab('thumbnails')}
                  ...
                  <button 
                    onClick={() => openSidebarWithTab('search')}
                    className="inline-flex items-center relative gap-2 font-semibold justify-center whitespace-nowrap rounded-md text-sm ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 hover:bg-muted hover:text-accent-foreground bg-transparent h-9 w-9"
              ...
              <div className="flex-1 relative overflow-hidden">
                ...
                <EnhancedSidebar isOpen={showSidebar} activeTab={sidebarActiveTab} onTabChange={setSidebarActiveTab} />
